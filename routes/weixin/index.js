        ...
        response.on('data',function(chunk){
            ...
            var userRequest = http.request(user_options, function (response) {
                ...
                response.on('data', function(chunk){
                    ...
                    var insert_sql = "insert into user(user_id,is_member,member_datetime,nickname,sex,headimgurl,be_date) values('"+user_id+"',0,0,'"+nickname+"',"+sex+",'"+headimgurl+"',Now())";
                    var update_sql = "update user set nickname='"+nickname+"',sex="+sex+",headimgurl='"+headimgurl+"' where user_id='"+user_id+"'";

                    var insert_uservalue_sql = "insert into user_value(user_id) values('"+user_id+"')";
                            ...
                            if(valls.length>0){//存在该用户数据,更新
                                ...
                                query(insert_sql, function (qerr, valls, fields) {
                                    if(qerr){
                                        ...
                                        query(insert_uservalue_sql,function (qerr,valls,fields) {
                                            if(qerr){
